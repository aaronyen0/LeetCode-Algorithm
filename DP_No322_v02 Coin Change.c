/**
 * 看到有人用BFS解這題，思路很有意思，我們都知道BFS的order理論上來說極高，因為它會掃過所有可能
 * 看起來是Order大約是(M / avg(coin[])) ^ N
 * 敘述一下別人的寫法
 * 1.將coins排序(超過amount的數字可以丟棄)
 * 2.從大面額的硬幣開始用BFS方法逐漸往小面額硬幣開始掃
 * 3.遇到的第一個滿足amount的點就是最小硬幣數 (大多數硬幣是大面額的，只有少數是其他較小面額的概念，因此硬幣少)
 *
 * 這個方法worst case (也就是沒有任何amount滿足需求)約 O(M ^ N) (每退一層多檢查M個數字，共N層)
 * 不過看起來多數測試應該在前幾層就抓到值了
 * 跟DP ( O(M * N) )比起來是有好有壞
 */
